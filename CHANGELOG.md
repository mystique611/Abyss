...
- **Wording fix**: "No marine life observations logged during this run." → "...during this dive." (Logbook and AquaDex empty states).
- **AquaDex**: added a rarity filter (All / Common / Epic / Ultra-Rare) that applies across every category alongside the existing category filter.
- **Mobile bottom nav bar enlarged ~50%** (padding, icons, labels) for easier tapping on phones, with iOS safe-area support so it's no longer cropped by the home-indicator area on notched devices.
